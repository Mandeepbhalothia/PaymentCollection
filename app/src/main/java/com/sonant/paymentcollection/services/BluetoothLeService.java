...
public class BluetoothLeService extends Service {
    ...
    public final static String BATTERY_DATA_AVAILABLE =
            "BATTERY_DATA_AVAILABLE";
    public final static String EXTRA_DATA_HEART_RATE =
            "EXTRA_DATA_HEART_RATE";
    ...
    public boolean connectToDevice(final String address) {
        ...
        if (bluetoothManager!=null) {
            if (bluetoothManager.getConnectedDevices(BluetoothGatt.GATT).contains(device)) {
                Log.d(TAG, "connectToDevice: already connected");
                return true;
            }
        }

        mGatt = device.connectGatt(this, true, gattClientCallback);
        Log.d(TAG, "connectToDevice: Connecting to selected device");
    ...
    private BluetoothGattCallback gattClientCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (newState == BluetoothGatt.STATE_CONNECTED){
                ...
                String device1 = sharedPreferences.getString("Dev_1",null);
...
//            setBatteryNotification(gatt);
    ...
    private void broadcastUpdate(final String action,
        ...
        if (HEART_RATE_MEASUREMENT_CHAR_UUID.equals(characteristic.getUuid())) {
            ...
            intent = new Intent(BATTERY_DATA_AVAILABLE);
            final int batteryLevel = characteristic.getIntValue
                    (BluetoothGattCharacteristic.FORMAT_UINT8, 0);

            if (batteryLevel != 0) {
                Log.d(TAG, String.format("Received battery level right: %d", batteryLevel));
            }
            intent.putExtra(BATTERY_DATA_AVAILABLE,batteryLevel);
        }
        if (intent!=null) {
            sendBroadcast(intent);
        }
